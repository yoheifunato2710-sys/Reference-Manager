...
export default function PaperManager() {
  ...
  // 見出しで区切られた次のセクション本文を取得（番号付き・なし両対応、境界は次の見出しまたは末尾）
  const nextSection = (text, afterIndex) => {
    const rest = text.slice(afterIndex);
    const nextHead = rest.match(/\n(?:\d+\.\s*)?(?:Title|Abstract|Introduction|Methods|Results|要旨|目的|方法|結果)/i);
    const end = nextHead ? nextHead.index : rest.length;
    return rest.slice(0, end).trim();
  };

  const hasCJK = (s) => /[\u3040-\u309f\u30a0-\u30ff\u4e00-\u9faf]/.test(s || "");
  ...
  const extractFromTextWithoutAI = () => {
    ...
    // 「1. Title」セクション：英語タイトルと日本語タイトルの両方を取り出す
    const titleSectionMatch = raw.match(/(?:^|\n)1\.\s*Title\s*\n([\s\S]*?)(?=\n\d+\.\s|\n(?:Abstract|要旨)|$)/i);
    if (titleSectionMatch) {
      const titleBlock = titleSectionMatch[1].trim();
      const blockLines = titleBlock.split(/\n/).map((l) => l.trim()).filter(Boolean);
      const firstLine = blockLines[0] || "";

      // 1) 同一行内の括弧：英語（日本語）または 英語 (日本語)。括弧内の空白を許容
      const paren = firstLine.match(/^(.+?)\s*[（(]\s*([^）)]+)\s*[）)]\s*$/);
      if (paren) {
        result.title = paren[1].trim();
        result.titleJa = paren[2].trim();
      } else if (blockLines.length >= 2) {
        // 2) 2行以上：1行目＝英語・2行目＝日本語 または 1行目＝日本語・2行目＝英語 を判定
        const a = blockLines[0];
        const b = blockLines[1];
        if (hasCJK(b) && !hasCJK(a)) {
          result.title = a;
          result.titleJa = b;
        } else if (hasCJK(a) && !hasCJK(b)) {
          result.title = b;
          result.titleJa = a;
        } else {
          result.title = firstLine;
          if (hasCJK(b)) result.titleJa = b;
        }
      } else {
        result.title = firstLine;
        // 「日本語タイトル」「和訳」などラベル付きの行が他にある場合
        const jaLabel = titleBlock.match(/(?:日本語タイトル|和訳|日本語)\s*[：:]\s*(\S[\s\S]*?)(?=\n|$)/i);
        if (jaLabel) result.titleJa = jaLabel[1].trim();
      }
    }
    // 先頭行が「1. Title」でない場合：先頭1行または2行から取得
    if (!result.title && lines.length > 0) {
      const first = lines[0].trim();
      if (first && !/^\d+\.\s/.test(first) && first.length < 500) {
        const p = first.match(/^(.+?)\s*[（(]\s*([^）)]+)\s*[）)]\s*$/);
        if (p) {
          result.title = p[1].trim();
          result.titleJa = p[2].trim();
        } else {
          result.title = first;
          if (lines.length >= 2) {
            const second = lines[1].trim();
            if (second && hasCJK(second) && second.length < 500) result.titleJa = second;
          }
        }
      }
    }

    // Abstract（要旨）：2. Abstract / 2. Abstract (要旨) / Abstract / 要旨 など
    const abstractHead = raw.match(/(?:^|\n)(?:\d+\.\s*)?(?:Abstract\s*(?:\(要旨\))?|要旨)\s*\n/i);
    if (abstractHead) {
      const content = nextSection(raw, abstractHead.index + abstractHead[0].length);
      if (content) result.abstract = content.slice(0, 10000);
    }
    if (!result.abstract) {
      const legacyAbstract = raw.match(/\n2\.\s*Abstract\s*(?:\(要旨\))?\s*\n([\s\S]*?)(?=\n3\.|$)/i);
      if (legacyAbstract) result.abstract = legacyAbstract[1].trim().slice(0, 10000);
    }

    // Introduction & Purpose（Full Translation は任意）
    const introHead = raw.match(/(?:^|\n)(?:\d+\.\s*)?Introduction\s*&\s*Purpose\s*(?:\(Full Translation\))?\s*\n/i);
    if (introHead) {
      const content = nextSection(raw, introHead.index + introHead[0].length);
      if (content) result.introductionPurpose = content.slice(0, 15000);
    }
    if (!result.introductionPurpose) {
      const legacyIntro = raw.match(/\n(?:\d+\.\s*)?Introduction\s*&\s*Purpose\s*\(Full Translation\)\s*\n([\s\S]*?)(?=\n\d+\.\s|$)/i);
      if (legacyIntro) result.introductionPurpose = legacyIntro[1].trim().slice(0, 15000);
    }

    // Methods: Analysis Process
    const methodsHead = raw.match(/(?:^|\n)(?:\d+\.\s*)?Methods\s*:\s*Analysis\s+Process\s*\n/i) || raw.match(/(?:^|\n)(?:\d+\.\s*)?Methods:\s*Analysis Process\s*\n/i);
    if (methodsHead) {
      const content = nextSection(raw, methodsHead.index + methodsHead[0].length);
      if (content) result.methodsAnalysis = content.slice(0, 15000);
    }
    if (!result.methodsAnalysis) {
      const legacyMethods = raw.match(/\n(?:\d+\.\s*)?Methods:\s*Analysis Process\s*\n([\s\S]*?)(?=\n\d+\.\s|\nResults|$)/i);
      if (legacyMethods) result.methodsAnalysis = legacyMethods[1].trim().slice(0, 15000);
    }

    // Results (Corresponding to Purpose)
    const resultsHead = raw.match(/(?:^|\n)(?:\d+\.\s*)?Results\s*\(Corresponding to Purpose\)\s*\n/i);
    if (resultsHead) {
      const content = nextSection(raw, resultsHead.index + resultsHead[0].length);
      if (content) result.resultsPurpose = content.slice(0, 15000);
    }
    if (!result.resultsPurpose) {
      const legacyResults = raw.match(/\n(?:\d+\.\s*)?Results\s*\(Corresponding to Purpose\)\s*\n([\s\S]*?)(?=\n\d+\.\s|$)/i);
      if (legacyResults) result.resultsPurpose = legacyResults[1].trim().slice(0, 15000);
    }
    ...
    const refStyleM = raw.match(/(?:参考文献様式|Reference\s*style|引用様式)\s*[：:]\s*([^\n]+)/i);
    if (refStyleM) result.referenceStyle = refStyleM[1].trim().slice(0, 200);
...

{
  ...
  "doi": "DOI（不明ならnull）",
  "referenceStyle": "参考文献様式（Vancouver / APA 等、不明ならnull）",
...
${pasteText.slice(0, 12000)}`, 2000);
      ...
      setExtracted({
        ...info,
        referenceStyle: info.referenceStyle ?? "",
...
${pasteText.slice(0, 12000)}`;
  ...
  const savePaper = async () => {
    ...
    const np = {
      ...
      doi: extracted.doi || "",
      referenceStyle: extracted.referenceStyle || "",
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          ...
          {view === "detail" && (
            <div style={{ flex: 1, overflowY: "auto", padding: "26px 30px" }}>
              {/* 情報：タイトル・著者・雑誌・参考文献様式 */}
              <Block label="情報">
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  <div>
                    <div style={{ ...mono(9), color: "#3d5070", marginBottom: 2 }}>英語タイトル</div>
                    <div style={{ fontSize: 14, color: "#dde3ef", lineHeight: 1.5 }}>{selected.title || "—"}</div>
                  </div>
                  {selected.titleJa && (
                    <div>
                      <div style={{ ...mono(9), color: "#3d5070", marginBottom: 2 }}>日本語タイトル</div>
                      <div style={{ fontSize: 14, color: "#60a5fa", lineHeight: 1.5 }}>{selected.titleJa}</div>
                    </div>
                  )}
                  <div>
                    <div style={{ ...mono(9), color: "#3d5070", marginBottom: 2 }}>著者</div>
                    <div style={{ fontSize: 13, color: "#8fa0b8", fontStyle: "italic" }}>{selected.authors?.length ? selected.authors.join(", ") : "—"}</div>
                  </div>
                  <div>
                    <div style={{ ...mono(9), color: "#3d5070", marginBottom: 2 }}>雑誌名</div>
                    <div style={{ fontSize: 13, color: "#8fa0b8" }}>{selected.journal || "—"}</div>
                  </div>
                  <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                    {selected.year && (
                      <div>
                        <div style={{ ...mono(9), color: "#3d5070", marginBottom: 2 }}>年</div>
                        <div style={{ fontSize: 13, color: "#8fa0b8" }}>{selected.year}</div>
                      </div>
                    )}
                    {selected.doi && (
                      <div>
                        <div style={{ ...mono(9), color: "#3d5070", marginBottom: 2 }}>DOI</div>
                        <a href={`https://doi.org/${selected.doi}`} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: "#60a5fa", fontFamily: "'JetBrains Mono',monospace" }}>{selected.doi} ↗</a>
                      </div>
                    )}
                    {(selected.referenceStyle || "").trim() && (
                      <div>
                        <div style={{ ...mono(9), color: "#3d5070", marginBottom: 2 }}>参考文献様式</div>
                        <div style={{ fontSize: 13, color: "#8fa0b8" }}>{selected.referenceStyle}</div>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
                <span onClick={() => updatePaper(selected.id, { starred: !selected.starred })}
                  style={{ cursor: "pointer", fontSize: 18, color: selected.starred ? "#fbbf24" : "#1a2235" }}>
                  {selected.starred ? "★" : "☆"}
                </span>
                <div style={{ marginBottom: 0 }}>{selected.tags.map((t) => <span key={t} className="tag">{t}</span>)}</div>
              </div>
              ...
              {(selected.pdfFile || selected.pdfPath) && (
                <div style={{ marginBottom: 18 }}>
                  <button className="gh" onClick={() => setView("pdf")}
                    style={{ ...mono(11), color: "#60a5fa", background: "rgba(96,165,250,0.12)", border: "1px solid rgba(96,165,250,0.35)", borderRadius: 6, cursor: "pointer", padding: "8px 14px", display: "inline-flex", alignItems: "center", gap: 6 }}>
                    📄 PDFを開く
                  </button>
                </div>
              )}
...
本研究は、深層学習（DL）ベースの画像増強アルゴリズムを用いた...`}
              ...
              {extracted && (
                <div className="fade-in" style={{ background: "#07090f", border: "1px solid #1a2235", borderRadius: 8, padding: "16px 18px", marginBottom: 20 }}>
                  ...
                  <div style={{ marginBottom: 10 }}>
                    <div style={{ ...mono(9), color: "#3d5070", marginBottom: 3 }}>参考文献様式</div>
                    <input value={extracted.referenceStyle || ""} onChange={(e) => setExtracted((d) => ({ ...d, referenceStyle: e.target.value }))}
                      placeholder="Vancouver, APA など"
                      style={{ ...SI, fontSize: 12 }} />
                  </div>
                  <div style={{ marginBottom: 10 }}>
